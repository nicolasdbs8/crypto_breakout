import pandas as pd
from strategy import entry_signal, macro_filter, rank_score
...
def run_backtest(data: dict, portfolio, btc_symbol: str = "BTC", debug_index: bool = False):
    """
    ...
    This variant disables the BTC overlay entirely (ALTS-only engine).
    """
    ...
    for i, date in enumerate(date_list):
        ...
        #    - exit_SMA100: if close(t-1) < SMA100(t-1) then exit at OPEN(t)
        # ---------------------------------------------------------------------
        for sym in list(portfolio.positions.keys()):
            if sym not in data:
                continue
            ...
            sma100_prev = df.loc[prev_date, "SMA100"] if "SMA100" in df.columns else None
            close_prev = float(df.loc[prev_date, "close"])
            if sma100_prev is not None and sma100_prev == sma100_prev and close_prev < float(sma100_prev):
                if sym in open_prices:
                    portfolio.exit(date, sym, open_prices[sym], reason="exit_SMA100")
        ...
        # C2) Alt entries (use entry_signal at prev_date)
        ...
        #    - stop_LL50: if low(t) <= LL50(t-1) -> exit at LL50(t-1)
        # ---------------------------------------------------------------------
    ...
    risk_df.to_csv(output_path_str("risk_frac_daily.csv"))
